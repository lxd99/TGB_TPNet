...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        set_random_seed(
            seed=run,
            deterministic_alg=args.use_random_projection or args.model_name == "NAT",
        )
        set_thread(3)
        ...
        if args.use_random_projection:
            random_projections = RandomProjectionModule(
                node_num=node_raw_features.shape[0],
        ...
        if args.model_name == "TGAT":
            ...
            dynamic_backbone = TPNet(
                ...
                dropout=args.dropout,
                device=args.device,
                not_embedding=args.not_embedding,
            )
            ...
            dynamic_backbone = NAT(
                n_feat=node_raw_features,
                e_feat=edge_raw_features,
                time_dim=args.time_feat_dim,
                output_dim=args.output_dim,
                num_neighbors=[1] + args.nat_num_neighbors,
                dropout=args.dropout,
                n_hops=args.num_layers,
                ngh_dim=args.nat_ngh_dim,
                device=args.device,
            )
    ...
    if args.num_runs > 1:
        ...
        wandb_logger.log_final(
            val_metrics=val_metric_all_runs, test_metrics=test_metric_all_runs
        )
